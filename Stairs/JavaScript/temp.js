function drawBolt(par) {
	...
	*/
	...
	if (!par.headType) par.headType = "потай";
	if ((par.len == 40 || params.boltHead == "hexagon") && par.headType != "меб.") par.headType = "шестигр.";
	var headHeight = 4;
	...
	if (!$sceneStruct.vl_1.realColors) {
		boltMaterial = params.materials.inox;
	}
	var boltLen = par.len;
	...
	if ($sceneStruct.vl_1.boltHead) {
		...
		var geom = new THREE.ExtrudeGeometry(shape, extrudeOptions);
		geom.applyMatrix(new THREE.Matrix4().makeTranslation(0, 0, 0));
		var nut = new THREE.Mesh(geom, params.materials.inox);
...
function calcPerpParams(line_p1, line_p2, p0) {
	...
	//функция рассчитывае параметры перпендикуляра, опущенного из точки p0 на прямую, заданную точками line_p1 и line_p2
...
function drawPolygon(polygonParams) {
	/*рисует правильный многоугольник со скругленными краями*/

	polygonParams.path = new THREE.Path();
	polygonParams.shape = new THREE.Shape();
	var dxfPrimitivesArr = polygonParams.dxfPrimitivesArr;
	var dxfBasePoint = polygonParams.dxfBasePoint;
	var edgeAngle = 2 * Math.PI / polygonParams.vertexAmt;
	var p0 = polygonParams.basePoint;
	var vertexes = [];
	vertexes[0] = p0;
	var stepAngle = Math.PI;

	//углы без учета скруглений
	for (var i = 1; i < polygonParams.vertexAmt; i++) {
		stepAngle = stepAngle - edgeAngle;
		vertexes[i] = polar(vertexes[i - 1], stepAngle, polygonParams.edgeLength);
	}

	var filletParams = {
		vertexes: vertexes,
		cornerRad: polygonParams.cornerRad,
		dxfBasePoint: polygonParams.dxfBasePoint,
		dxfPrimitivesArr: polygonParams.dxfPrimitivesArr,
		type: polygonParams.type,
	}

	polygonParams.path = fiiletPathCorners(filletParams);
	polygonParams.vertexes = vertexes;

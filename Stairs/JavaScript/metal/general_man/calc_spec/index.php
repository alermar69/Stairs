...
if($method == 'POST'){
	...
	$sheet->getColumnDimension('C')->setWidth(12.48);
	$sheet->getColumnDimension('D')->setWidth(38.12);
...
function printTable(&$sheet, $caption, $data = [], $startRow = 5){
	...
	$sheet->getStyle("A{$oldStartRow}:D{$startRow}")->applyFromArray($style_wrap);
    $oldStartRow++;
    $sheet->getStyle("A{$oldStartRow}:D{$startRow}")->applyFromArray($arial10);
	return ($startRow+2);
}

...
function drawMarshTreads2(par) {
	...
	*/
	...
	if (par.stairAmt > 0) {
		...
		//коррекция толщины
		if (params.stairType == "лотки" || params.stairType == "рифленая сталь" || params.stairType == "пресснастил") plateParams.thk = 4;
		...
		for (var i = startIndex; i < par.stairAmt; i++) {
			...
			if (i == startIndex) {
				if (params.calcType == "mono" && hasTurnRack && par.marshId > 1 && getMarshParams(par.marshId).botTurn == 'забег') {
					...
					tread.position.y = par.h * (startIndex + 1);
...
function drawPlatform2(par) {
	...
	*/
	...
	if (params.stairType == 'дпк') plateParams.material = params.materials.dpc;
...
function calcWndTread1Points(par){
	...
	if (par.hasTurnRack && par.isTread) {
		...
		if ((par.treadId == 3 || par.treadId == 6) && params.calcType == 'lt-ko') {
...
function drawWndTread1(par) {
	...
	*/
	...
	//лотки
	if(params.stairType == "лотки" || params.stairType == "рифленая сталь" || params.stairType == "пресснастил") extrudeOptions.amount = 4;
    ...
    */
	...
	if (params.riserType == "есть") {
		...
		if (hasTurnRack) {//Меняем размер подступенка, riserFix рассчитывается в calcWndTread1Points
			riserPar.cutWndIn = 95 / Math.cos(treadParams[3].edgeAngle) - riserPar.thk * Math.tan(riserPar.ang) + 0.03;
		}
		//отрисовка
		riserPar = drawRectRiser(riserPar);
		riser = riserPar.mesh;
...
function calcMarshNotches(marshId){
    ...
    var model = params.model;
		...
		if (params.stairModel == 'Прямая' && params.calcType !== 'timber') {
    ...
    //снизу поворот
    if(marshPar.botTurn != "пол"){
        if((model == "косоуры") && (railingSide == "внешнее" || railingSide == "две")){
            notches.botOut = {
                x: params.nose,
        ...
        if(model == "косоуры" || model == "тетива+косоур"){
            notches.botIn = {
				...
				if (marshPar.botTurn == 'забег' && (model == 'тетивы' || model == 'тетива+косоур')) {
					notches.botOut = {
							x: params.nose,
							y: params.rackSize / 2
					};
    ...
    if(marshPar.topTurn != "пол"){
        ...
        if(model == "тетивы"){
            notches.topIn = {x: marshPar.a - (marshPar.b - (params.rackSize - params.stringerThickness) / 2 + topMarshXOffset),
                y: (params.rackSize - params.stringerThickness) / 2
            };
...
function drawNotchedPlate(par){
    ...
    */
    var material = params.materials.timber;
...
function drawTimberPlt_G(par){
    ...
    par.notches = {
		...
		console.log(par)
    if (params.model == "косоуры" && (params.railingSide_1 == "внешнее" || params.railingSide_1 == "две") && getMarshParams(par.botMarshId).stairAmt > 0) {
...
function drawRectRiser(par) {
	...
	*/
	...
	if (par.ang) {
		...
		// console.log(par.thk / Math.cos(par.ang))
		// console.log(par.thk / Math.sin(par.ang))
		// console.log(par.thk / Math.tan(par.ang))
		// console.log(par.thk * Math.sin(par.ang))
		// console.log(par.thk * Math.tan(par.ang))
		// console.log(par.thk * Math.atan(par.ang))
		var inDelta = 0;//par.thk * Math.tan(-par.ang - Math.PI / 2);//par.thk / Math.cos(par.ang);
		console.log(inDelta)
		
		var p1 =  {x:inDelta,y:0}
		var p2 = newPoint_xy(p1, -inDelta, par.thk);
		var p3 = newPoint_xy(p2, par.len, 0);
		var p4 = newPoint_xy(p3, outDelta, -par.thk);
		var p5 = newPoint_xy(p4, -par.len - inDelta, 0);

...
function calcMarshEndPoint(pos, rotY, marshPar){
	var endPoint = {
	...
	var turnParams = calcTurnParams(botMarshId);
		...
		if(plusMarshDist){
			if(params.stairModel == "П-образная с забегом"){
				...
				if(params.calcType == "mono") endPoint.z += (params.marshDist - 40) * turnFactor;
				}
			if(params.stairModel == "П-образная трехмаршевая"){
				...
				if(params.model == "ко") endPoint.z = (params.M / 2 + params.marshDist - 25) * turnFactor;	//25 - подогнано
				if(params.calcType == "mono") endPoint.z = (params.M / 2 + params.marshDist - 45) * turnFactor;	//45 - подогнано
				}
...
function calcStaircaseMoove(lastMarshEnd){
	...
	if(params.stairModel == "Прямая") moove.x = moove.x / turnFactor;

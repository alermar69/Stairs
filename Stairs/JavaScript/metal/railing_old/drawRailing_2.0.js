...
function drawRailingSectionNewel(railingSectionParams) {
    ...
    var textHeight = railingSectionParams.textHeight;
    ...
    if (railingModel === "Ригели") {
        ...
        if (stairAmt !== 0) {
            ...
            rigelParams.length = stepLength * (stairAmt - 1) + (30 + rackParams.rackProfile / 2 + 65) / Math.cos(handrailAngle);
            ...
            if (topEnd === "площадка") {
                x0 = racks[racks.length - 2].x - rackParams.rackProfile / 2;//x0 + b1 * (stairAmt - 1);// + rackParams.rackProfile / 2;
                y0 = h1 + Math.sin(handrailAngle) * rigelParams.length;// + rackParams.rackProfile / Math.tan(handrailAngle);
                //y0 = h1 * stairAmt + rigelParams.poleProfileY + rackParams.rackProfile / Math.tan(handrailAngle);
                rigelParams.basePoint = [x0, y0, z0];
    ...
    if (handrail !== "нет") {
        ...
        var handrailParams = {
            ...
            platformLengthTop: platformLengthTop + b1 * (stairAmt - 1),
            h1: h1,
        ...
        if (stairAmt !== 0) {
            ...
            if (railingSectionParams.model === "ко") x0 += a1 - b1;
            ...
            handrailParams.yTop = racks[racks.length - 2].y - 90 + rackLength * scale - y0 + handrailProfileY;
...
function drawRailingSection(railingSectionParams) {
    ...
    if (railingModel === "Ригели" || railingModel === "Стекло на стойках") {
        ...
        if (stairAmt !== 0) {
            ...
            //    x = x0 + b1 * i * scale;
            //    y = y0 + h1 * i * scale;
            //    rackParams.basePoint = [x, y, z0];
            //    for (var j = 0; j < rackPositionStep.length; j++)
            //        if (i + 1 == rackPositionStep[j])
            //            rack = drawRack3d(rackParams).rackNumber;
            //}
            ...
            //if (topEnd === "площадка") {
            //    rackParams.basePoint[0] += 50 * scale;
            //    rack = drawRack3d(rackParams).rackNumber;
            ...
            //        rackParams.basePoint = newPoint_x(p0, rackDist * scale, 0);
            //        drawRack3d(rackParams);
            //    }

            //    /*последняя стойка площадки*/
            //    rackParams.basePoint = newPoint_x(rackParams.basePoint, (platformLengthTop - 50 - 70 - b1 / 2) * scale, 0);
            //    drawRack3d(rackParams);
            //}
    ...
    if (handrail !== "нет") {
        ...
        if (railingModel !== "Самонесущее стекло") {
            if (stairAmt !== 0) {
                ...
                handrail.position.z = -pole3DParams.poleProfileZ / 2 - rackParams.rackProfile / 2;

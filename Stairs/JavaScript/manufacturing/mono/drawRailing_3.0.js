...
function calculateGlassPoints(par){
	...
	if (par.key == 'in' && marshPar.topTurn !== 'нет' && !marshPar.lastMarsh) {
		var deltaX = marshTurnParams.pltExtraLen - par.treadOffset - par.glassThickness - 20;
		if (nextMarshPar.hasRailing.in) deltaX = 60 - marshTurnParams.pltExtraLen - 7.5;
...
function calcPltHoles(par, placeMod){
	...
	for (var i = 0; i < pltGlassAmt; i++){
		...
		if(par.botConnection && placeMod == 'bot' && i == pltGlassAmt - 1) center4.hasHolder = false;
...
function drawPolylineHandrail(par) {
	...
	for (var i = 0; i < points.length - 1; i++) {
		if (points[i] && points[i + 1]) {
			...
			if (i == 0 && p2.x != p1.x && (params.handrailEndType == "под углом")) {
			//if (i == 0 && p2.x != p1.x && (params.handrailEndType == "под углом" || par.isHandrailEndAng)) {

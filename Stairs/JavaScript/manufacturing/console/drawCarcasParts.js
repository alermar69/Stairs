...
function drawStringerConsoleFlan(par) {
	...
	var shapePar = {
		...
		dxfBasePoint: newPoint_xy(par.dxfBasePoint, 100,  - 100),
	}
	...
	if (typeof specObj != 'undefined') {
		...
		var name = par.heithFlan + "x" + par.widthFlan;
		var area = par.heithFlan * par.widthFlan / 1000000;
	...
	if (!(~par.type.indexOf("winder") || par.type == 'platformBotP')) {
		...
		var p0 = { x: 0, y: 0 };
		var p1 = newPoint_xy(p0, -heightBox / 2, -widthBox / 2);
		var p2 = newPoint_xy(p0, -heightBox / 2, widthBox / 2);
		var p3 = newPoint_xy(p0, heightBox / 2, widthBox / 2);
		var p4 = newPoint_xy(p0, heightBox / 2, -widthBox / 2);

		var pointsShape = [p1, p4, p3, p2];
		...
		box.position.x = par.widthFlan / 2;
		box.position.y = -par.heigthFlan / 2;
		box.position.z = 8;
	...
	if (~par.type.indexOf("winder")) {
		...
		if (par.type == "winder1" || par.type == "winder3") {
			...
			var line_p2_p3 = parallel(p2, p3, 100);
...
function calcStringerConsoleFlanPar(par) {
	...
	//par.heithFlan = 60 + offsetHoleY * 4;
	par.widthTread = 0;

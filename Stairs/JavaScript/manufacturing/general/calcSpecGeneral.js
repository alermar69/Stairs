...
function isPainting(item){
    ...
    if('metalPaint' in item && item.metalPaint){
		...
		var paintColor = params.carcasColor || 'не указано';
		if(params.metalPaint !== 'нет') paintType += ", цвет " + paintColor;
...
function getMaterialName(item){
	...
	if(params.calcType == "vint"){
		matName = params.treadMaterial;
		if(item.group == "handrails"){
			matName = params.handrail_bal;
			if(params.handrail_bal == "массив") matName = params.treadMaterial;
		}
...
function printSpecificationCollation(partsList, customTable) {
    ...
    $.each(partsList, function(articul, list){
		...
		var isModelData = "";
		// console.log(list.name, list.isModelData);
...
function printPoleList(){
	...
	// text += printPartsTable(poleTypes);
	// console.log(materialGroupedPoles)

...
function drawMarshTreads21(par) {
	/*
	par = {
		marshId
		dxfBasePoint
		...
		for (var i = startIndex; i < par.stairAmt; i++) {
			...
			if (drawRectTread) {
				if (params.stairType == 'дпк' || params.stairType == "лиственница тер.") {
					...
					if (params.calcType == 'bolz') {
						plateParams.holes = [];
						var center1 = { x: 20 + 40 / 2, y: 20 + 40 / 2 }
						var center2 = { x: 20 + 40 / 2, y: plateParams.width - (20 + 40 / 2) }
						if (turnFactor == 1) center1.x = center2.x = plateParams.len - (20 + 40 / 2)
						plateParams.holes.push({ center: center1, rad: 20 });
						plateParams.holes.push({center: center2, rad: 20 });
...
function drawRailingSectionNewel2(par) {
	...
	if (params.calcType === 'bolz' && par.key === "in") {
		//railingPositionZ = (params.M - calcTreadLen()) / 2 + rackProfile + 10;
...
*/
	...
	if (params.calcType === 'bolz') {
		mooveY = 90;
		sideOffset = (params.M - calcTreadLen()) / 2 + 40 + 10
		//if (turnFactor === -1) sideOffset = -40;
	}
